...
const Home = () => {
  ...
  return (
    ...
    <div className="home-container">
      ...
      <section id="minimax-regret" className="home-section">
        <h2>最大后悔最小化决策模型</h2>
        <p className="home-text-paragraph">
          最大后悔最小化模型（Minimax Regret）是一种帮助人们在面对不确定性和未来变化时做出稳健选择的决策方法。它源于统计决策理论，核心思想是：
          在做决定时，考虑每种选择在未来可能带来的“后悔值”，然后选择那个在所有可能情况下最大后悔值最小的选项。
        </p>
        <ul className="home-feature-list">
          <li><strong>定义后悔值</strong>：后悔值是指在某个未来情景下，如果选择了某个选项，与最佳选择之间的差距。它反映了“如果我当初选了另一个，会不会更好”的情绪。</li>
          <li><strong>构建决策矩阵</strong>：用户输入多个备选方案和多种可能的未来情景，系统自动计算每种组合下的结果。</li>
          <li><strong>计算最大后悔值</strong>：系统为每个备选方案，在不同情景下计算其后悔值，并找出该方案的最大后悔值。</li>
          <li><strong>选择最小化最大后悔的方案</strong>：系统推荐那个在所有备选方案中，最大后悔值最小的选项，帮助用户在不确定性中做出最不后悔的决定。</li>
          <li><strong>可视化分析</strong>：通过图表展示不同方案在不同情景下的表现，帮助用户直观理解选择背后的逻辑。</li>
        </ul>
        <p className="home-text-paragraph top-margin">
          这个模型特别适合用于那些未来结果不确定、但又希望尽量避免极端后悔的决策场景，如职业选择、投资决策、产品方向等。它不追求最优，但追求最不坏的结果。
        </p>
        <div className="home-button-container">
          <Link to="/minimax-regret-decision/list" className="home-link-button">我做的后悔分析</Link>
          <Link to="/minimax-regret-decision" className="home-link-button">去分析最大后悔</Link>
        </div>
      </section>
